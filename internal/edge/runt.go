...
func (r *RUNTAlgorithm) Analyze(ctx context.Context, input interface{}) (*AnalysisResult, error) {
	startTime := time.Now()
	
	pkg, ok := input.(*types.Package)
	if !ok {
		return nil, fmt.Errorf("RUNT: invalid input type, expected *types.Package")
	}
	...
	result := &AnalysisResult{
		...
		AttackVectors: []string{
			...
			"Semantic Typosquatting", 
			"Combinatorial Squatting",
		},
	...
	if len(pkg.Name) < r.config.MinPackageLength || len(pkg.Name) > r.config.MaxPackageLength {
	...
	suspiciousPackages := r.findSuspiciousPackages(pkg.Name)
	
	if len(suspiciousPackages) > 0 {
		// Compute overall threat score using Bayesian mixture model
		features := r.computeAllSimilarityFeatures(pkg.Name, suspiciousPackages[0].Name)
		threatScore := r.computeBayesianThreatScore(features)
		
		result.ThreatScore = threatScore
		result.Confidence = r.computeConfidence(features, suspiciousPackages)
		...
		for _, suspicious := range suspiciousPackages {
			if suspicious.SimilarityScore > r.config.OverallThreshold {
				finding := Finding{
					...
					Description: fmt.Sprintf("Package name '%s' is suspiciously similar to '%s'", pkg.Name, suspicious.Name),
					Evidence: map[string]interface{}{
						"target_package":     suspicious.Name,
						"similarity_score":   suspicious.SimilarityScore,
						"similarity_features": suspicious.Features,
						"attack_type":        suspicious.AttackType,
					},
					Remediation: "Verify package authenticity and consider using exact package names",
				}
		...
		// Add metadata
		result.Metadata["suspicious_packages_count"] = len(suspiciousPackages)
		result.Metadata["max_similarity_score"] = suspiciousPackages[0].SimilarityScore
		result.Metadata["similarity_features"] = features
	} else {
		result.ThreatScore = 0.0
		result.Confidence = 0.8 // Lower confidence when no similar packages found
	}
