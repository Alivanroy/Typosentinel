...
// FeatureStatistics type defined in enhanced_training_data.go
...
func (fe *FeatureEngineer) NormalizeFeatures(featureSet *FeatureSet) error {
	// Extract features and names from FeatureSet
	features := make([]float64, 0, len(featureSet.Features))
	names := make([]string, 0, len(featureSet.Features))
	...
	// Normalize features
	normalizedFeatures := fe.normalizer.Normalize(features, names)

	// Update the FeatureSet with normalized values
	for i, name := range names {
		featureSet.Features[name] = normalizedFeatures[i]
	}
...
// Normalize method moved to advanced_feature_extractor.go to avoid duplication
