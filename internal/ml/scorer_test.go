...
func TestAdvancedScorerInterface(t *testing.T) {
	// Test that AdvancedScorer implements Scorer interface
	var _ Scorer = (*AdvancedScorer)(nil)

	scorer, err := NewAdvancedScorer(nil)
	if err != nil {
		t.Fatalf("NewAdvancedScorer failed: %v", err)
	}
	if scorer == nil {
		t.Fatal("NewAdvancedScorer returned nil")
	}
}

func TestAdvancedScorerScore(t *testing.T) {
	scorer, err := NewAdvancedScorer(nil)
	if err != nil {
		t.Fatalf("NewAdvancedScorer failed: %v", err)
	}
	...
	if result.Metadata["scorer_type"] != "advanced_ensemble" {
		t.Errorf("Expected scorer_type advanced_ensemble, got %v", result.Metadata["scorer_type"])
	}
}

func TestAdvancedScorerGetModelInfo(t *testing.T) {
	scorer, err := NewAdvancedScorer(nil)
	if err != nil {
		t.Fatalf("NewAdvancedScorer failed: %v", err)
	}
	...
	// Check advanced scorer specific fields
	if modelInfo.Type != "ensemble" {
		t.Errorf("Expected model type ensemble, got %s", modelInfo.Type)
	}

	if modelInfo.Name != "AdvancedEnsembleScorer" {
		t.Errorf("Expected model name AdvancedEnsembleScorer, got %s", modelInfo.Name)
	}
}

func TestAdvancedScorerGetThresholds(t *testing.T) {
	scorer, err := NewAdvancedScorer(nil)
	if err != nil {
		t.Fatalf("NewAdvancedScorer failed: %v", err)
	}
...
func TestAdvancedScorerUpdateModel(t *testing.T) {
	scorer, err := NewAdvancedScorer(nil)
	if err != nil {
		t.Fatalf("NewAdvancedScorer failed: %v", err)
	}
	...
	updateData := map[string]interface{}{
		...
		"ensemble_weights": map[string]float64{
		...
		"model_info": map[string]interface{}{
			"name":          "Updated Advanced Scorer",
			"version":       "1.1.0",
			"description":   "Updated ensemble model",
			"type":          "ensemble",
			"training_time": time.Now().Format(time.RFC3339),
			"feature_count": 20,
			"metrics": map[string]float64{
	...
	modelInfo := scorer.GetModelInfo()
	if modelInfo.Name != "Updated Advanced Scorer" {
		t.Errorf("Expected model name 'Updated Advanced Scorer', got %s", modelInfo.Name)
	}
...
func TestAdvancedScorerWithCustomConfig(t *testing.T) {
	customConfig := &ScorerConfig{
	...
	scorer, err := NewAdvancedScorer(customConfig)
	if err != nil {
		t.Fatalf("NewAdvancedScorer with custom config failed: %v", err)
	}

	thresholds := scorer.GetThresholds()
	if thresholds.Malicious != 0.9 {
		t.Errorf("Expected custom malicious threshold 0.9, got %f", thresholds.Malicious)
	}

	if thresholds.Suspicious != 0.6 {
		t.Errorf("Expected custom suspicious threshold 0.6, got %f", thresholds.Suspicious)
	}

	if thresholds.MinConfidence != 0.5 {
		t.Errorf("Expected custom min confidence 0.5, got %f", thresholds.MinConfidence)
	}
}
