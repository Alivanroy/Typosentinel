...
func TestNewBitbucketConnector(t *testing.T) {
	...
	connector, err := NewBitbucketConnector(config)
	if err != nil {
		t.Fatalf("Failed to create Bitbucket connector: %v", err)
	}
	...
	if connector.GetPlatformName() != "Bitbucket" {
		t.Errorf("Expected platform name 'Bitbucket', got '%s'", connector.GetPlatformName())
	}
	...
	if connector.GetAPIVersion() != "2.0" {
		t.Errorf("Expected API version '2.0', got '%s'", connector.GetAPIVersion())
	}
}

func TestBitbucketConnector_URLEscaping(t *testing.T) {
	config := repository.PlatformConfig{
		BaseURL: "https://api.bitbucket.org/2.0",
		Auth: repository.AuthConfig{
			Token: "test-token",
...
func TestBitbucketConnector_Close(t *testing.T) {
	config := repository.PlatformConfig{
		BaseURL: "https://api.bitbucket.org/2.0",
		Auth: repository.AuthConfig{
			Token: "test-token",
...
func TestBitbucketConnector_ParseFullName(t *testing.T) {
	config := repository.PlatformConfig{
		BaseURL: "https://api.bitbucket.org/2.0",
		Auth: repository.AuthConfig{
			Token: "test-token",
...
func TestBitbucketConnector_MatchesFilter(t *testing.T) {
	config := repository.PlatformConfig{
		BaseURL: "https://api.bitbucket.org/2.0",
		Auth: repository.AuthConfig{
			Token: "test-token",
