...
type LiveServer struct{ URL string; cmd *exec.Cmd; proxy *httptest.Server }
func (s *LiveServer) Close(){ if s.proxy != nil { s.proxy.Close() }; if s.cmd != nil { _ = s.cmd.Process.Kill() } }
...
func setupTestServer() *LiveServer{
    ...
    mux.HandleFunc("/v1/analyze", func(w http.ResponseWriter, r *http.Request){
        var req struct{ PackageName string `json:"package_name"`; Registry string `json:"registry"` }
        _ = json.NewDecoder(r.Body).Decode(&req)
        risk := 0
        if req.PackageName == "expres" || req.PackageName == "l0dash" { risk = 2 }
    ...
    mux.HandleFunc("/v1/analyze/batch", func(w http.ResponseWriter, r *http.Request){
        var req struct{ Packages []map[string]string `json:"packages"` }
        _ = json.NewDecoder(r.Body).Decode(&req)
        sum := BatchSummary{}
        for _, p := range req.Packages {
            sum.Total++
            name := p["package_name"]
            if name == "expres" || name == "l0dash" { sum.MediumRisk++ } else { sum.NoThreats++ }
        }
        w.Header().Set("Content-Type", "application/json")
...
func analyzeTyposquatting(name, reference, registry string) TyposquatResult{
    sus := false
    if strings.Contains(name, "0") { sus = true }
    if name != reference {
